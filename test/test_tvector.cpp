...
TEST(TDynamicVector, copied_vector_is_equal_to_source_one)
{
	...
	for (size_t i = 0; i < v1.size(); ++i) v1[i] = 1;
	TDynamicVector<int> v2(v1);
	EXPECT_TRUE(v2 == v1);
}

TEST(TDynamicVector, copied_vector_has_its_own_memory)
{ // Че мы хотим сделать: создавть вектор, от него скопировать и изменить исходный. дальше проверяем равны ли эл-ты
	TDynamicVector<int> v(3);
	v[0] = 1; v[1] = 2; v[2] = 3;
	TDynamicVector<int> v2(v);
	// изменяем исходный вектор
	v[0] =9999;
	EXPECT_NE(v2[0], v[0]);
}
...
// как я поняла, дальше тесты не на гетер сетеры а на эт и индексацию
...
{
	TDynamicVector<int> v1(3), v2(3);
	for (size_t i = 0; i < 3; ++i) { v1[i] = 2; v2[i] = 2; }
	EXPECT_TRUE(v1 == v2);
}
